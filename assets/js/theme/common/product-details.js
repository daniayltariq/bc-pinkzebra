...
import { existsKitInCart, isUnderJoinProcess } from './utils/scentflirt-products';
import { isProductAvailableForSubscription } from './utils/product-scentflirt-label';
import { addAvailabilityToProductsCards } from './utils/product-availability';
...
export default class ProductDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
        ...
        addAvailabilityToProductsCards('.productView', 'PDP');
    }
    ...
    productOptionsChanged(event) {
        ...
        utils.api.productAttributes.optionChange(productId, $form.serialize(), 'products/bulk-discount-rates', async (err, response) => {
            const productAttributesData = response.data || {};
            const productAttributesContent = response.content || {};
            const neededProducts = [{ sku: productAttributesData.sku }];
            this.updateProductAttributes(productAttributesData);
            this.updateView(productAttributesData, productAttributesContent);
            this.updateProductDetailsData();
            bannerUtils.dispatchProductBannerEvent(productAttributesData);
            event.preventDefault();
            await trackInventoryLevel({
                productsResults: neededProducts,
                storefrontAPIToken: this.context.storefrontAPIToken,
                pageType: this.context.pageType,
                identifier: event.target.tagName,
                outOfStock: this.context.outOfStock,
    ...
    async addProductToCart(event, form) {
        ...
        this.$overlay.show();
        const isAnyKitItem = await existsKitInCart(this.context);
        const isScentflirtProduct = isProductAvailableForSubscription(this.context.productCustomFields || []);
        const isJoinProcess = await isUnderJoinProcess(this.context);

        if ((isAnyKitItem && !isScentflirtProduct) || isJoinProcess) {
            $addToCartBtn
                .val(originalBtnVal)
                .prop('disabled', false);
            return showAlertModal('Cannot add items to cart, please checkout or remove the item/s to continue');
        }
