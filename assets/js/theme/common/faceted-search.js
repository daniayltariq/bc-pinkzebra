...
import nod from './nod';
import trackLayoutChanges from '../global/track-layout-changes';
import { PriceFacet } from './select-price-facet';
import { addAvailabilityToProductsCards } from './utils/product-availability';
import { initScentflirtProductLabels } from './utils/product-scentflirt-label';
...
class FacetedSearch {
     ...
     */
    constructor(context, requestOptions, callback, options) {
        // Private properties
        ...
        this.priceFacet = new PriceFacet(context, 'price-option');
    ...
    updateView() {
        ...
        api.getPage(urlUtils.getUrl(), this.requestOptions, async (err, content) => {
            ...
            const neededProducts = $('#product-listing-container').find($('li.product article'));
            await initScentflirtProductLabels(this.context, '.productGrid');
            await trackLayoutChanges({ productsResults: neededProducts, storefrontAPIToken: this.requestOptions.context.storefrontAPIToken });
        });
    }
